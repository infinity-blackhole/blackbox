...
**Dependencies:** `aes`, `cbc`, `lz4`, `rmp-serde`, `serde`, `bytes`, `blackbox-core`, `blackbox-schemas`
...
### `gen-entities`
**Input:** `schemas.json` → **Output:** `crates/blackbox-schemas/src/lib.rs`
...
(consume blackbox-master-data + blackbox-schemas, no reverse dependencies)
```
