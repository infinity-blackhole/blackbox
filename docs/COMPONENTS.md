...
├── schemas/
│   ├── quest.schema              # Entity definitions for quest tables
│   ├── gacha.schema              # Entity definitions for gacha tables
│   ├── user.schema               # Entity definitions for user/profile tables
│   ├── item.schema               # Entity definitions for items/materials/weapons
│   ├── battle.schema             # Entity definitions for battle/quest flow
│   └── shared.schema             # Common enums and shared types
├── crates/
│   ├── blackbox-core/            # Shared types, errors, config, clock
│   ├── blackbox-schemas/         # Generated entity structs from schemas.json
│   ├── blackbox-schemas-parser/  # Parses schemas.json into IR
│   ├── blackbox-schemas-codegen/ # Generates Rust source from IR
...
└── tools/
    └── blackbox-masterdata/      # Master data tool (dump, patch, inspect, validate)
```
...
schemas.json (single file, 607 tables)
    ↓ parsed by blackbox-schemas-parser
SchemaIR (intermediate representation)
    ↓ codegen by blackbox-schemas-codegen
    ↓
crates/blackbox-schemas/src/generated/structs.rs        (struct + enum definitions)
crates/blackbox-schemas/src/generated/deserialize.rs    (positional msgpack Deserialize)
crates/blackbox-schemas/src/generated/enums.rs          (From<i32> for enum types)
    ↓
crates/blackbox-schemas/src/lib.rs (include! all generated)
```

The generated `Deserialize` impl reads msgpack arrays positionally by index, matching the binary format exactly. The generated code is written to `src/generated/` (gitignored) and included at compile time. This gives full rust-analyzer support with autocomplete and type checking.
...
### `blackbox-schemas` — Generated Entity Structs

**Dependencies:** `serde`, `rmp-serde`

**Provides:**
- One `#[derive(Serialize, Deserialize)]` struct per master data table (~200 total).
- Enum types as `#[derive(Debug, Clone, Copy, PartialEq, Eq)]` with `i32` representation.
- All structs use positional msgpack deserialization via custom `Deserialize` impl
  that reads array elements by their schema field index.

**Generated by:** `build.rs` which calls `blackbox-schemas-parser` → `blackbox-schemas-codegen`.
The generated code lives in `src/generated/` (gitignored) and is included via `include!()`.

**Relationships:** Depended on by `blackbox-master-data`, `blackbox-diff-sync`,
`blackbox-store`, `blackbox-game-server`. No internal dependencies.

---

### `blackbox-schemas-parser` — Schema JSON Parser

**Dependencies:** `serde`, `serde_json`

**Provides:**
- `parse_schemas(path: &Path) -> Result<SchemaIR>` — reads `schemas.json` and produces
  an intermediate representation.
- `SchemaIR` — the intermediate representation containing:
  - `tables: Vec<TableDef>` — each with `table_key`, `class_name`, `columns: Vec<ColumnDef>`
  - `enums: Vec<EnumDef>` — each with `name`, `variants: Vec<(String, i32)>`
- `ColumnDef` — `index: u32`, `name: String`, `raw_type: String`, `is_enum: bool`
- Validation: checks for duplicate table keys, duplicate column indices, unknown enum references.

**Relationships:** Depended on by `blackbox-schemas` (build.rs). No dependencies on other workspace crates.

---

### `blackbox-schemas-codegen` — Rust Code Generator

**Dependencies:** `quote`, `syn`, `proc-macro2`

**Provides:**
- `generate_schemas(ir: &SchemaIR) -> TokenStream` — generates all Rust source code.
- Generates three files:
  - `structs.rs` — struct definitions and enum definitions
  - `deserialize.rs` — `impl Deserialize<'de> for EntityMXxx` using positional array reading
  - `enums.rs` — `impl From<i32> for EnumName` and `impl TryFrom<i32> for EnumName`
- The generated `Deserialize` impl uses a custom `PositionalDeserializer` that reads
  each msgpack value by index and converts to the appropriate type.

**Relationships:** Depended on by `blackbox-schemas` (build.rs). No dependencies on other workspace crates.

---

### `blackbox-master-data` — Binary Parsing Pipeline

**Dependencies:** `aes`, `cbc`, `lz4`, `rmp-serde`, `serde`, `bytes`, `blackbox-core`, `blackbox-schemas`, `blackbox-schemas-parser`
...
**Relationships:** Depends on `blackbox-core`, `blackbox-schemas`. Used by
`blackbox-game-server` (startup), `blackbox-admin` (reload webhook), and tools.
...
### `blackbox-masterdata` — Master Data CLI

**Dependencies:** `clap`, `blackbox-core`, `blackbox-master-data`, `blackbox-schemas`

**Provides:** Single CLI binary for all master data operations:

```
blackbox-masterdata dump --input bin.e/ --output ./dump/
    Dump all tables to JSON files (one per table)

blackbox-masterdata patch --input bin.e/ --output patched.bin.e
    Extend time-gated content to 2030, deduplicate campaigns, fix schedules/seasons

blackbox-masterdata inspect --input bin.e/ --table m_quest
    Print table summary: row count, column names, sample rows

blackbox-masterdata validate --input bin.e/
    Validate binary integrity: check offsets, verify LZ4 decompression, count rows

blackbox-masterdata search --input bin.e/ --table m_quest --column QuestId --value 101
    Search for rows matching a specific column value
```

**Relationships:** Depends on `blackbox-core`, `blackbox-master-data`, `blackbox-schemas`.
...
Tool: blackbox-masterdata
(consumes blackbox-master-data + blackbox-schemas, no reverse dependencies)
```
