...
**Relationships:** Depended on by `master-data`, `diff-engine`, `store`, `grpc-server`.
...
**Relationships:** Depends on `core`, `schemas`. Used by `grpc-server` (startup),
...
**Relationships:** Depends on `core`. Used by `grpc-server`, `auth-server`, `diff-engine`.
...
**Dependencies:** `axum`, `sqlx`, `tokio`, `hmac`, `sha2`, `core`

**Provides:**
- `GET /` — OAuth token validation.
- `GET /me` — `{ id, name }` from token.
- `GET /check-username` — username availability.
- `AuthStore` — sqlx SQLite (`auth.db`).
- `TokenService` — HMAC-based token signing/validation.

**Relationships:** Depends on `core`. Called by `grpc-server` for FB token validation.
...
**Relationships:** Depended on by `cdn-server`. No internal dependencies.
...
```
                    ...
                    └──────┬───────┘
                           │
┌──────────┐    ┌──────────▼───────┐    ┌──────────────┐
│  schemas  │◄───│  master-data     │    │  cdn-server  │
└────┬─────┘    └──────────┬───────┘    └──────────────┘
     │                     │
     │            ┌────────▼───────┐
     │            │     api        │
...
Client ──gRPC──► game-server
                    ...
                    └──► auth-server (HTTP: FB token validation)

Client ──HTTP──► cdn-server
